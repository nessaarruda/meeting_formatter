...
var app = {
  init: function() {
    ...
    chrome.runtime.sendMessage({fn: "getTemplate"}, function(response){
      userInput1.value = response[1];
    });
    chrome.runtime.sendMessage({fn: "getTemplate"}, function(response){
      userInput2.value = response[2];
    });
    chrome.runtime.sendMessage({fn: "getTemplate"}, function(response){
      userInput3.value = response[3];
    });
    chrome.runtime.sendMessage({fn: "getTemplate"}, function(response){
      userInput4.value = response[4];
    });
    saveTemplate.addEventListener("click", function(){
      chrome.runtime.sendMessage({fn: "setTemplate", name: name.value, userInput1: userInput1.value, userInput2: userInput2.value, userInput3: userInput3.value, userInput4: userInput4.value});
    });
  }
};
